...
export const UNREACHABLE =
  "Couldn't reach the agent service on port 8000. Start it with " +
  "`uvicorn service.main:app --port 8000` and try again.";

export async function fetchProducts(): Promise<Product[] | null> {
  const data = await api.get<{ products: Product[] }>("/products", { limit: "60" });
