...
Copied from the reference host and unmodified below the docstring. ``RedisSessionStore`` in
``service/redis_sessions.py`` puts the six storage methods over Redis, which is what this
deployment runs: the previous build on this project kept conversation history in a
process-local dict and recorded the consequence as its first fragility -- state resets on
redeploy and splits across instances. The in-memory store here is the test double.
